...
public class EmailService(IOptions<SmtpSettings> settings, IConfiguration configuration, UserManager<User> userManager)
{
  ...
  public async Task SendEmailConfirmationToken(User user)
  {
    ...
    message.AppendFormat("<a href=\"{0}\">Verify Email Address</a>", configuration["WebClientUrl"] + "/confirm-email?userId=" + user.Id + "&token=" + token);
  ...
  public async Task SendPasswordResetToken(User user)
  {
    ...
    message.AppendFormat("<a href=\"{0}\" target=\"_blank\">Reset Password</a>", configuration["WebClientUrl"] + "/reset-password?email=" + user.Email + "&token=" + token);
